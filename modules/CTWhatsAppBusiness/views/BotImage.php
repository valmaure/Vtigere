...
if (isset($_FILES)) {
	...
	$filename = $_FILES[$name]['name'];
		$currenUserID = $current_user->id;
 		if($filename){
			$whatsappFolder = "modules/CTWhatsAppBusiness/CTWhatsAppBusinessStorage/";
			$whatsappFolderPath = "../CTWhatsAppBusinessStorage/";
			$year  = date('Y');
			$month = date('F');
			$day   = date('j');
			$week  = '';
			...
			echo  $newfileURL = $site_URL.$whatsappFolder . "$year/$month/$week/".$time.$filename;
