...
class CTWhatsAppBusiness_WhatsappBot_View extends Vtiger_Index_View {
    ...
    function WhatsappBotList(Vtiger_Request $request) {
		...
		$getBotLicenseDetail = CTWhatsAppBusiness_WhatsappBot_View::getBotLicenseDetail();
		...
		$settingWhatsAppModules = 'Settings:CTWhatsAppBusiness';
        ...
        $totalSendMonthMessage = CTWhatsAppBusiness_WhatsappBot_View::getMonthRecordCount();
	...
	function getBotLicenseDetail(){
		global $adb;
		$licensevalidation = $adb->pquery("SELECT * FROM vtiger_ctwhatsappbusiness_botlicense");
	...
	function whatsAppBotLicenseDeactive(Vtiger_Request $request){
		...
		if($licenseRows == 1){
			...
			if($result_response['message']){
				$adb->pquery("DELETE FROM vtiger_ctwhatsappbusiness_botlicense", array());
	...
	function saveWhatsAppBotLicense(Vtiger_Request $request){
		...
		$expirydate = Settings_CTWhatsAppBusiness_SaveLicense_Action::encrypt_decrypt($result_response['expirydate'],'e');
		...
		if($status == 1){
			if($licenseRows == '1'){
				$query = $adb->pquery("UPDATE vtiger_ctwhatsappbusiness_botlicense SET licensekey=?, expirydate=?, status=?, sendmessagelimit=?, domain=? WHERE 1",array($licenseKey, $expirydate, $status, $sendmessagelimit, $site_URL));
			}else{
				$query = $adb->pquery("INSERT INTO vtiger_ctwhatsappbusiness_botlicense (licensekey,expirydate,status,sendmessagelimit,domain) values(?,?,?,?,?)",array($licenseKey, $expirydate, $status, $sendmessagelimit, $site_URL));
			}
		}else if($status == 2){
			if($licenseRows == '1'){
				$status = 1;
	...
	function getUserWhatsAppNumber(Vtiger_Request $request){
		...
		$query = $adb->pquery("SELECT * FROM vtiger_ctwhatsappbusinessusers WHERE whatsappstatus = '1' and customfield5 = ?", array($assignuserid));
		...
		for ($i=0; $i < $rows; $i++) { 
			...
			if($remove != 1){
				...
				if($multipleWahtsapp == 'multipleWhatsapp'){
					$scanNumberQuery = $adb->pquery("SELECT * FROM vtiger_ctwhatsappbusinessusers WHERE whatsappno = ?", array($scanwhatsapp_number));
				...
				if($botid){
					$botQuery = "SELECT * FROM vtiger_ctwhatsappbusiness INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_ctwhatsappbusiness.ctwhatsappid WHERE vtiger_ctwhatsappbusiness.whatsapp_botid = ? AND vtiger_crmentity.deleted = 0";
					$totalBotCountQuery = $adb->pquery($botQuery, array($botid));
					$totalbotmessage = $adb->num_rows($totalBotCountQuery);
					if($totalbotmessage){
						$lasteMessageDateTime = '';
						$lasteBotDateTimeQuery = $adb->pquery($botQuery." ORDER BY vtiger_ctwhatsappbusiness.ctwhatsappid DESC LIMIT 1", array($botid));
				...
				$allBots[] = array('checkbox'=>'<input type="checkbox" class="activeBot" data-scannumber="'.$scanwhatsapp_number.'" data-whatsappbotstatus="'.$whatsappbotstatus.'" data-botid="'.$botid.'" '.$checked.' name="activeBot" id="activeBot" value="'.$deleted.'">','botname'=>'<a href="index.php?module=CTWhatsAppBusiness&view=WhatsappBot&mode=WhatsappBotConfiguration&botRecordId='.$botid.'">'.$botname.'</a>','assignuserid'=>$username,'status'=>$status,
					'scanwhatsapp_number'=>$scanwhatsapp_number,'totalbotmessage'=>$totalbotmessage,'lastemessagedatetime'=>$lasteMessageDateTime,'action'=>'<a href="index.php?module=CTWhatsAppBusiness&view=List&viewname=103&orderby=&sortorder=&app=SALES&search_params=[[[%22whatsapp_botid%22%2C%22c%22%2C%22'.$botid.'%22]]]" target="_blank">
									<img src="layouts/v7/modules/CTWhatsAppBusiness/image/botlog.png" title="'.vtranslate('LBL_CHATLOG',$moduleName).'" style="width: 15px;">
								</a>&nbsp;&nbsp;<a href="index.php?module=CTWhatsAppBusiness&view=WhatsappBot&mode=WhatsappBotConfiguration&duplicateRecordId='.$botid.'">
									<i class="fa fa-clone" title="'.vtranslate('LBL_DUPLICATE',$moduleName).'"></i>
								</a>&nbsp;&nbsp;
								<a href="index.php?module=CTWhatsAppBusiness&view=WhatsappBot&mode=WhatsappBotConfiguration&botRecordId='.$botid.'">
								...
								</span>', 'customerResponseCount' => '<a href="index.php?module=CTWhatsAppBusiness&view=WhatsappChat&mode=allWhatsAppMSG&customerResponse=1">'.$customerResponseCount.'</a>');
			}
		}
	...
	function WhatsappBotConfiguration(Vtiger_Request $request) {
		...
		$query = $adb->pquery("SELECT * FROM vtiger_ctwhatsappbusinessusers WHERE whatsappstatus = '1' and customfield5 = ?", array($currentUserID));
	...
	function getWhatsappBotFlowData(Vtiger_Request $request){
		...
		if($assignuserchange == 1){
			...
			if($botID){
				$adb->pquery("UPDATE vtiger_ctwhatsappbusiness SET whatsapp_botid = ? WHERE whatsapp_botid = ?", array($botID, $botRecordid));
			}
			if($duplicateBot){
				$adb->pquery("UPDATE vtiger_ctwhatsappbusiness SET whatsapp_botid = '' WHERE whatsapp_botid = ?", array($botID));	
			}
	...
	function scanQRCodeInPopup(Vtiger_Request $request) {
		...
		if($whatsappbotstatus == 0){
			...
			$fields = array(
				'domain' => $site_URL,
				"url" => $site_URL.'/modules/CTWhatsAppBusiness/CTWhatAppReceiverBot.php?userid='.$currenUserID,
				"licenceKey" => 'crmtiger12*',
				"statusurl" => $site_URL.'/modules/CTWhatsAppBusiness/WhatsappStatusBot.php?userid='.$currenUserID,
			);
    ...
    public function logoutWhatsApp(Vtiger_Request $request){
		...
		header("Location: index.php?module=CTWhatsAppBusiness&view=WhatsappBot&mode=WhatsappBotList");
    }
    ...
    function whatsAppBotButtonSetting(Vtiger_Request $request) {
		...
		$multipleWhatsAppNumber = CTWhatsAppBusiness_Record_Model::getAllUserWhatsappNumber($currentUserID);
		...
		$viewer->assign('IMAGEDIRECTORY', '/modules/CTWhatsAppBusiness/CTWhatsAppBusinessStorage/BotButton/');
	...
	function SaveWhatsAppBotButton(Vtiger_Request $request) {
		...
		$target = $root_directory.'/modules/CTWhatsAppBusiness/CTWhatsAppBusinessStorage/BotButton/';
		...
		header("Location: index.php?module=CTWhatsAppBusiness&view=WhatsappBot&mode=whatsAppBotButtonSetting");
	}
